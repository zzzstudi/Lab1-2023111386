...
public class Main {
    ...
    public static Double calPageRank(String word) {
        ...
        */
        ...
        // ===== 改进的初始化：使用TF-IDF思想 =====
        // TF：词频（单词出现次数）
        // IDF：逆文档频率（这里用每个单词作为"文档"，计算有多少个单词指向它）
        ...
        for (int iter = 0; iter < maxIterations; iter++) {
            ...
            for (String node : graph.keySet()) {
                double sum = 0.0;
                Node currentNode = graph.get(node);
    ...
    public static void saveGraphAsMermaid() {
        try {
            ...
            try (PrintWriter writer = new PrintWriter(new OutputStreamWriter(
                    new FileOutputStream("output/graph.md"), "UTF-8"))) {
                ...
                for (Map.Entry<String, Node> entry : graph.entrySet()) {
                    ...
                    writer.printf("  %s[\"%s<br/>出现%d次\"]\n", safeNode, node, n.frequency);
                }
                writer.println();
